...
- **Backend**: Wrap `find_reviewers()` in FastAPI. Add persistence, job queue, and WebSocket for live progress. The authoritative institution list is `INSTITUTIONS` in `reviewer_finder_agent.py`.
- **Containerization**: Three services — `backend` (FastAPI + uvicorn), `frontend` (Nginx serving built Vite bundle + reverse-proxying `/api` and `/ws` to backend), `db` (MariaDB). Orchestrated by a single `docker-compose.yml`. Same stack runs locally and in the cloud — no separate dev/prod configs.
...
| `submitForReview()` simulating 4-second state transition | Real API call to `POST /matching/start` + WebSocket progress |
...
- Job status tracking and live progress (WebSocket streaming agent log messages).
...
PeerLink/
  backend/
    ...
    ws.py                     # WebSocket /ws/matching/{job_id}
  ...
  frontend/                   # Ported from Figma Make (mock data stripped)
    src/
      ...
      api/                    # NEW: API client
      lib/                    # NEW: WebSocket hook, types
    vite.config.ts            # Dev proxy /api and /ws to backend
    package.json
    Dockerfile                # Multi-stage: node:20 build → nginx:alpine serve
    nginx.conf                # SPA fallback + reverse-proxy /api and /ws to backend service
    .dockerignore
  docker/
    mariadb/
  ...
  src/                        # Existing Python (unchanged)
    ...
    agent/tools.py
    agent/prompt.py
  requirements.txt            # Remove streamlit; add: fastapi, uvicorn[standard], websockets, sqlmodel, pymysql, pypdf
...
| `frontend` | Built from `frontend/Dockerfile` | `80:80` (exposed) | `backend` | Multi-stage: `node:20-alpine` runs `npm ci && npm run build`; `nginx:alpine` serves `/dist`. `nginx.conf` handles SPA fallback and reverse-proxies `/api/*` → `backend:8000` and `/ws/*` with `Upgrade`/`Connection` headers for WebSocket. |
...
5. Install deps in `requirements.txt`: `fastapi`, `uvicorn[standard]`, `websockets`, `sqlmodel`, `pymysql`, `pypdf`
...
7. **WebSocket**: browser devtools → Network → WS → verify progress frames stream during matching; verify Nginx correctly upgrades the connection.
8. **Manual entry**: submit form → appears in `/abstracts` list with status `unmatched`.
9. **Programs**: verify program filter dropdown shows the 3 real GF award types (not the CAP/NIAP/ESPD mock labels).
10. **Self-contained image check**: build images on a clean machine (no source bind-mounts needed) and confirm the stack still runs end-to-end — this is exactly how the future CI/CD pipeline will ship them to the cloud.
