...
async def _run_one_ephemeral(
    ...
    """Run agent for one institution without writing to job_storage."""
    async with _SEMAPHORE:
        try:
            ...
            logger.error("Ephemeral matching failed for %s: %s", institution, exc)
...
async def run_ephemeral(
    ...
    """Run the agent on the fly and return reviewers — no DB writes."""
